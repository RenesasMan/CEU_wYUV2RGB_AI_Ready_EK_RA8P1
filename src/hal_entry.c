...
extern uint8_t g_image_vga_sram[VGA_WIDTH * VGA_HEIGHT * BYTE_PER_PIXEL];
extern uint8_t g_image_vga_sram_rgb8888[VGA_WIDTH * VGA_HEIGHT * 4U];
...
void hal_entry(void)
{
    ...
    volatile uint32_t loop_index, index_max;
    volatile uint8_t Y0_val, Y1_val, Cr_val, Cb_val;
...
//        err = YUV422toRGB888(&g_image_vga_sram_rgb8888, &gp_image_buffer, g_image_width, g_image_height );
//        handle_error(err, "** ceu_operation FAILED **\r\n");
        index_max = sizeof(g_image_vga_sram);
        for( loop_index = 0; loop_index<sizeof(g_image_vga_sram); loop_index+=4)
        {
            Cr_val = g_image_vga_sram[loop_index+2]; // 0x8A
            Cb_val = g_image_vga_sram[loop_index+0]; // 0x70
            Y1_val = g_image_vga_sram[loop_index+3]; // 0x7E
            Y0_val = g_image_vga_sram[loop_index+1]; // 0x7C
            __NOP();
        }
